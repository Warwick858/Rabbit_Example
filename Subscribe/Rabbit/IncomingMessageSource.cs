...
{
	public class IncomingMessageSource : IObservable<IncomingMessage<BunnyModel>>
	{
		private readonly IModel _channel;
		private readonly JsonSerializerSettings _jsonSettings;
		private readonly string _queue;

		public IncomingMessageSource(IModel channel, string queue)
		{
			_channel = channel;
			_queue = queue;
		...
		public IDisposable Subscribe(IObserver<IncomingMessage<BunnyModel>> observer)
		{
		...
		private IncomingMessage<BunnyModel> CreateIncomingMessage(EventPattern<BasicDeliverEventArgs> receivedEvent)
		{
			IncomingMessage<BunnyModel> newMsg = null;
			...
			if (envelope != null)
			{
				try
				{
					newMsg = new IncomingMessage<BunnyModel>(
